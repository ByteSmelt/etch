...
suite "Global Edge Buffer - Integration with Heap":
  ...
  test "Heap Operations: addRef populates EdgeBuffer":
    ...
    # Create reference: id1 -> id2 by storing it in the table's fields
    ...
    # EdgeBuffer uses lazy update - need to trigger cycle detection to populate it
    discard heap.detectCycles(forceFull = true)
  ...
  test "Heap Operations: freeObject clears EdgeBuffer":
    ...
    # Store references in the table's fields
    let obj1 = heap.objects[id1]
    obj1.fields["child1"] = V(kind: vkRef, refId: id2)
    obj1.fields["child2"] = V(kind: vkRef, refId: id3)
    heap.trackRef(id1, V(kind: vkRef, refId: id2))
    heap.trackRef(id1, V(kind: vkRef, refId: id3))

    echo &"Created objects with edges: #{id1}->#{id2}, #{id1}->#{id3}"

    # EdgeBuffer uses lazy update - trigger cycle detection to populate it
    discard heap.detectCycles(forceFull = true)
...
suite "Global Edge Buffer - Cycle Detection":
  ...
  test "Cycle Detection: Simple cycle with EdgeBuffer":
    ...
    # Create cycle by storing references in table fields
    let objA = heap.objects[idA]
    let objB = heap.objects[idB]
    objA.fields["next"] = V(kind: vkRef, refId: idB)
    objB.fields["next"] = V(kind: vkRef, refId: idA)
    ...
    objA.strongRefs = 1  # Only referenced by B
    objB.strongRefs = 1  # Only referenced by A

    # Run cycle detection (this also populates EdgeBuffer via lazy update)
    let cycles = heap.detectCycles(forceFull = true)

    # Verify EdgeBuffer has the edges (after cycle detection)
    var edgesA: seq[int] = @[]
  ...
  test "Cycle Detection: Complex cycle with EdgeBuffer":
    ...
    # Create cycle chain by storing references in table fields
    let obj1 = heap.objects[id1]
    let obj2 = heap.objects[id2]
    let obj3 = heap.objects[id3]
    let obj4 = heap.objects[id4]
    obj1.fields["next"] = V(kind: vkRef, refId: id2)
    obj2.fields["next"] = V(kind: vkRef, refId: id3)
    obj3.fields["next"] = V(kind: vkRef, refId: id4)
    obj4.fields["next"] = V(kind: vkRef, refId: id1)
    heap.trackRef(id1, V(kind: vkRef, refId: id2))
    ...
    obj1.strongRefs = 1  # Will be in cycle
    obj2.strongRefs = 1
    obj3.strongRefs = 1
    obj4.strongRefs = 1

    # Run cycle detection (this populates EdgeBuffer)
    let cycles = heap.detectCycles(forceFull = true)

    # Verify EdgeBuffer has all edges (after cycle detection)
  ...
  test "Cycle Detection: No false positives":
    ...
    # Store references in the table fields
    let rootObj = heap.objects[root]
    rootObj.fields["childA"] = V(kind: vkRef, refId: idA)
    rootObj.fields["childB"] = V(kind: vkRef, refId: idB)
...
suite "Global Edge Buffer - Edge Cases":
  ...
  test "Edge Case: Self-referencing object":
    ...
    # Create self-reference by storing in table fields
    let obj1 = heap.objects[id1]
    obj1.fields["self"] = V(kind: vkRef, refId: id1)
    ...
    # Check EdgeBuffer after cycle detection
    var foundSelf = false
    for target in heap.edgeBuffer.outgoingEdges(id1):
      if target == id1:
        foundSelf = true
