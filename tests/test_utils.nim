...
import std/[os, osproc, strutils, strformat, streams, times, monotimes]
...
proc compileEtchFile*(etchExe: string, etchFile: string): bool =
  ## Compile an Etch file and return true if successful
  ## This ensures the .etcx bytecode file is generated
  ## Note: Running without --run will compile and cache the bytecode
  let (output, exitCode) = execCmdEx(&"{etchExe} --gen vm {etchFile}")
  if exitCode != 0:
    echo &"Failed to compile {etchFile}: {output}"
    return false
  return true
...
proc runDebugServerWithInput*(etchExe: string, testProg: string, inputCommands: string, timeoutSecs: int = 5): tuple[output: string, exitCode: int] =
  ...
  let process = startProcess(
    etchExe,
    args = args,
    options = {poUsePath}  # Don't merge stderr into stdout to keep JSON clean
  )
  ...
  try:
    ...
    # Read output with timeout (use monotonic time for wall-clock timeout)
    var output = ""
    let startTime = getMonoTime()
    let timeoutDuration = initDuration(seconds = timeoutSecs)
    ...
    let elapsed = getMonoTime() - startTime
    let remainingTime = max(0, int((timeoutDuration - elapsed).inMilliseconds))
