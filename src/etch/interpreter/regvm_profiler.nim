...
type
  ...
  RegVMProfiler* = ref object
    enabled*: bool
    startTime*: Time
    executionEndTime*: Time
    totalInstructions*: uint64
    instructionProfiles*: Table[RegOpCode, InstructionProfile]
...
proc newProfiler*(): RegVMProfiler =
  result = RegVMProfiler(
    ...
    totalInstructions: 0,
    instructionProfiles: initTable[RegOpCode, InstructionProfile](),
...
proc generateReport*(profiler: RegVMProfiler): string =
  ...
  if profiler.sourceLocationProfiles.len > 0:
    type AggregatedLocation = object
      ...
      totalCount: uint64
      totalTime: Duration
      instructions: Table[RegOpCode, tuple[count: uint64, time: Duration]]
    ...
    for locProfile in profiler.sourceLocationProfiles:
      ...
      if not aggregated.hasKey(key):
        aggregated[key] = AggregatedLocation(
          ...
          totalCount: 0,
          totalTime: initDuration(),
          instructions: initTable[RegOpCode, tuple[count: uint64, time: Duration]]()
        )
    ...
    for i, aggLoc in sortedAggregated:
      ...
      var instrList: seq[tuple[op: RegOpCode, count: uint64, time: Duration]] = @[]
      for op, data in aggLoc.instructions:
        instrList.add((op: op, count: data.count, time: data.time))

      instrList.sort(proc(a, b: tuple[op: RegOpCode, count: uint64, time: Duration]): int =
