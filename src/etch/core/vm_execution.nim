...
proc execute*(vm: VirtualMachine, verbose: bool = false): int =
  ...
  let hasHooks = vm.verboseLogging or
    ...
    vm.replayEngine != nil or
    (vm.isProfiling and vm.profiler != nil) or
  ...
  when not defined(deploy):
    ...
    # Ensure cleanup happens on all exit paths (normal return, early return, exception)
    defer:
      # Only cleanup if we're at the top level (frames.len <= 1)
      if vm.frames.len <= 1:
  ...
  # Flush any remaining buffered output and run final cleanup
  if vm.frames.len <= 1:
    flushOutput(vm)
