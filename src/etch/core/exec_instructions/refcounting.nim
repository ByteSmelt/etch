...
proc execNewWeak*(vm: VirtualMachine, instr: Instruction, verbose: bool) {.inline.} =
  ...
  # Handle nil refs - weak ref to nil is just 0
  if targetVal.kind == vkNil:
    setReg(vm, instr.a, makeWeak(0))
    logVM(verbose, &"opNewWeak: created weak ref to nil")
    return

  assert targetVal.isRef, &"opNewWeak: target in R[{instr.b}] is not a ref (got {targetVal.kind})"
...
proc execWeakToStrong*(vm: VirtualMachine, instr: Instruction, verbose: bool) {.inline.} =
  ...
  # strongId == 0 means the target was freed or the weak ref was to nil
  # In this case, we return nil
  if strongId == 0:
    setReg(vm, instr.a, V(kind: vkNil))
    logVM(verbose, &"opWeakToStrong: weak #{weakVal.weakId} target was freed or nil, returning nil")
  else:
