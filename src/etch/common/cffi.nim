...
proc etchTypeToFFIType(typ: types.TypeKind): ptr libffi.Type =
  ...
  ## Note: addr is used because the imported C types require bypassing
  ## Nim's strict pointer type checking, especially on Windows
  case typ
  of tkVoid:
    return cast[ptr libffi.Type](addr type_void)
  of tkInt:
    return cast[ptr libffi.Type](addr type_sint64)
  of tkFloat:
    return cast[ptr libffi.Type](addr type_double)
  of tkBool:
    return cast[ptr libffi.Type](addr type_uint8)
  of tkString:
    return cast[ptr libffi.Type](addr type_pointer)
  else:
