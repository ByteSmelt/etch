...
proc emitInstruction(gen: var CGenerator, instr: Instruction, debugInfo: DebugInfo, pc: int) =
  ...
  of opEqI:
    ...
    gen.emit(&"if ({cond}(r[{regIdx}], etch_make_int({imm}))) goto L{pc + 2};  // EqI")
  ...
  of opLtI:
    ...
    gen.emit(&"if ({cond}(r[{regIdx}], etch_make_int({imm}))) goto L{pc + 2};  // LtI")
  ...
  of opLeI:
    ...
    gen.emit(&"if ({cond}(r[{regIdx}], etch_make_int({imm}))) goto L{pc + 2};  // LeI")
