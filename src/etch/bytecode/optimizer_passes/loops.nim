...
proc canHoistInstruction(pc: int, instr: Instruction, loop: LoopRegion,
  ...
  if writtenReg.isSome:
    ...
    # CRITICAL FIX: Check if the destination register is clobbered by ANY other
    # instruction inside the loop (not just loop-carried dependencies)
    if dataFlow.defs.hasKey(reg):
      for def in dataFlow.defs[reg]:
        # If there's another definition of this register inside the loop
        # (other than the instruction we're trying to hoist), we can't hoist
        if def.pc != pc and def.pc >= loop.startPC and def.pc <= loop.endPC:
          return false
