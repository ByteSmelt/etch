        ...
        if usesConst and int(constIdx) < constants.len:
          ...
          if constVal.kind == vkInt and fitsIn8BitSigned(constVal.ival):
            ...
            if newOp != opNoOp:
              ...
              # Create new Instruction instead of modifying discriminant
              let converted = Instruction(
                op: newOp,
                a: instr.a,
                opType: ifmtABx,
                bx: uint16(reg) or (uint16(imm8) shl 8)
              )

              res[i].instr = converted
              # Mark the LoadK as NoOp since we no longer need it
              res[i-1].instr = Instruction(op: opNoOp, a: 0, opType: ifmtABC, b: 0, c: 0)
              convertedCount.inc
