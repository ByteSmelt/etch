...
import std/[os, strformat, strutils, algorithm, osproc, sequtils, times]
...
proc runSingleCTest*(testFile: string): TestResult =
  ## Run a single test file using C backend
  let baseName = testFile.splitFile.name
  let directory = testFile.splitFile.dir
  let resultFile = directory / baseName & ".pass"
  let errorFile = directory / baseName & ".fail"
  ...
  # Generate unique executable name with timestamp to avoid conflicts
  let timestamp = getTime().toUnix()
  let exeName = binDir / fmt"{baseName}_{timestamp}_c"
  let cFileName = binDir / fmt"{baseName}_{timestamp}.c"

  # Execute the test using C backend
  let etchExe = getAppFilename()

  # First, generate C code
  let genCmd = fmt"{etchExe} --emit c {testFile}"
  let genResult = executeWithSeparateStreams(genCmd)

  if genResult.exitCode != 0:
    # Compilation to C failed
    if hasErrorFile:
      # Expected failure - compare error output
      result.actual = smartFilterOutput(genResult)
      result.passed = result.expected == result.actual
      if not result.passed:
        result.error = "Error output mismatch"
    else:
      # Unexpected failure
      result.error = fmt"Failed to generate C code: {smartFilterOutput(genResult)}"
      result.actual = smartFilterOutput(genResult)
  ...
  # Move generated C file to bin directory
  let sourceC = testFile.changeFileExt("c")
  if fileExists(sourceC):
    moveFile(sourceC, cFileName)
  else:
    result.error = "C file was not generated"
    return
  ...
  let compileCmd = if sdkPath.len > 0:
    fmt"clang -isysroot {sdkPath} -O2 -o {exeName} {cFileName}"
  else:
    fmt"clang -O2 -o {exeName} {cFileName}"

  let compileResult = executeWithSeparateStreams(compileCmd)

  if compileResult.exitCode != 0:
    # C compilation failed
    if hasErrorFile:
      # Maybe this was expected?
      result.actual = "C compilation failed"
      result.passed = false
      result.error = fmt"C compilation failed: {smartFilterOutput(compileResult)}"
    else:
      result.error = fmt"C compilation failed: {smartFilterOutput(compileResult)}"
      result.actual = smartFilterOutput(compileResult)
  ...
  let runCmd = fmt"./{exeName}"
  let execResult = executeWithSeparateStreams(runCmd)
...
proc runCTests*(path: string = "examples"): int =
  ## Run tests using C backend - if path is a file, run single test; if directory, run all tests

  # Check if path is a file or directory
  if fileExists(path):
    # Single file test
    echo fmt"Running single C backend test: {path}"

    let res = runSingleCTest(path)
    ...
    # Directory test
    echo fmt"Running C backend tests in directory: {path}"
    ...
    for testFile in testFiles:
      echo fmt"Running {testFile.splitFile.name}... "
      let res = runSingleCTest(testFile)
      results.add(res)
    ...
    echo fmt"C Backend Test Summary: {passed} passed, {failed} failed, {testFiles.len} total"
