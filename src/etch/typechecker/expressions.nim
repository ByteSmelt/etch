...
proc inferMatchExpr(prog: Program; fd: FunDecl; sc: Scope; e: Expr; subst: var TySubst): EtchType =
  # Forward declaration - actual implementation in statements.nim will override
  if e.typ != nil:
    return e.typ
  return tVoid()
...
proc inferIfExpr(prog: Program; fd: FunDecl; sc: Scope; e: Expr; subst: var TySubst): EtchType =
  ...
  let thenType = if e.ifThen.len == 0:
    tVoid()
  elif e.ifThen[^1].kind == skExpr:
    inferExprTypes(prog, fd, sc, e.ifThen[^1].sexpr, subst)
  else:
    tVoid()
  ...
  for elifCase in e.ifElifChain:
    ...
    let elifType = if elifCase.body.len == 0:
      tVoid()
    elif elifCase.body[^1].kind == skExpr:
      inferExprTypes(prog, fd, sc, elifCase.body[^1].sexpr, subst)
    else:
      tVoid()
  ...
  let elseType = if e.ifElse.len == 0:
    tVoid()
  elif e.ifElse[^1].kind == skExpr:
    inferExprTypes(prog, fd, sc, e.ifElse[^1].sexpr, subst)
  else:
    tVoid()
