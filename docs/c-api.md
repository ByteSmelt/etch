...
Etch provides full debugging support for embedded scripts through the Debug Adapter Protocol (DAP), enabling VSCode integration with breakpoints, stepping, and variable inspection.

### Debug Server API

#### Creating a Debug Server

```c
// Create debug server from compiled program
...
**Requirements:**
- Context must have been compiled with debug mode enabled (`debug=1`)
- Program must be already compiled via `etch_compile_file` or `etch_compile_string`

#### Handling Debug Requests

```c
// Process a DAP request and return response
...
- `next` - Step over (execute current line)
- `stepIn` - Step into function calls
- `stepOut` - Step out of current function
- `pause` - Pause execution
- `stackTrace` - Get call stack frames
- `scopes` - Get variable scopes (locals, globals, registers)
- `variables` - Get variable values for a scope
- `threads` - Get thread list
- `disconnect` - End debug session

### Creating a Debug-Enabled Application

Create a C application that acts as a DAP server:
...
int main(int argc, char** argv) {
    ...
    // Compile the script
    if (etch_compile_file(ctx, argv[1]) != 0) {
    ...
    // Create debug server
    EtchDebugServer server = etch_debug_server_new(ctx, argv[1]);
    if (!server) {
...
### Compiling a Debug-Enabled Application

```bash
# Linux
gcc -Wall -Wextra -I./include -o my_debugger my_debugger.c \
    -L./lib -letch -lm -Wl,-rpath,$ORIGIN/lib

# macOS
clang -Wall -Wextra -I./include -o my_debugger my_debugger.c \
    -L./lib -letch -lm -Wl,-rpath,@executable_path/lib
```
  ...
  "configurations": [
    {
      "type": "etch",
      "request": "launch",
      "name": "Debug Embedded Etch",
      "program": "${workspaceFolder}/examples/capi/test_debug.etch",
      "debugExecutable": "${workspaceFolder}/my_debugger",
      "stopOnEntry": true
    }
  ]
}
...
- `program`: Path to the Etch script to debug
- `debugExecutable`: Path to your debug-enabled binary (optional, defaults to Etch compiler)
- `stopOnEntry`: Pause at first line (default: true)
- `trace`: Enable DAP protocol logging (default: true)

### Using the Debugger

1. Open your Etch script in VSCode
...
The debugger provides three scopes:

1. **Local Variables**
   - Variables in current function scope
   - Only shows variables that are defined and in scope at current PC
   - Handles shadowing and nested scopes correctly
   - Marks uninitialized variables as `<uninitialized>`

2. **Globals**
   - All global variables defined in the program

3. **Registers (Debug)**
   - First 16 VM registers with their values
   - Shows which variable is using each register
...
```
┌─────────────┐         DAP Protocol (JSON)          ┌──────────────────┐
│   VSCode    │ ◄──────────────────────────────────► │  Your C Binary   │
│  Debugger   │            stdin/stdout              │  (my_debugger)   │
└─────────────┘                                      └──────────────────┘
                                                              │
                                                              │ C API
                                                              ▼
...
**Communication Flow:**
1. VSCode sends DAP requests (JSON) to your binary via stdin
2. Your binary calls `etch_debug_server_handle_request`
3. Debug server processes request and controls VM execution
4. Debug server returns DAP response (JSON)
5. Your binary sends response to VSCode via stdout
6. VSCode updates debugger UI

### Troubleshooting

#### Binary Not Found
- Verify `debugExecutable` path is correct
- Check binary has execute permissions (`chmod +x my_debugger`)
- Use absolute paths or ensure binary is in PATH

#### Breakpoints Not Hit
- Ensure debug mode is enabled: `etch_context_new_with_options(0, 1)`
- Verify line numbers match compiled bytecode
- Set breakpoints before starting execution

#### Variables Show Wrong Values
- Current instruction may not have executed yet
- Check correct stack frame is selected
- Use "Registers (Debug)" to see raw VM state

#### No Response from Debug Server
- Check stderr for error messages
- Verify JSON requests are well-formed
- Ensure debug loop reads stdin and writes to stdout
